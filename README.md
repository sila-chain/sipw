...
The [SIP] validator that's one more than [`sipv`](https://github.com/sila/sipv).

```
USAGE:
    sipw [OPTIONS] [SOURCES]...
...
---
sip: 2
description: A really short example of an SIP.
title: Sample of an SIP
...
error[preamble-order]: preamble header `description` must come after `title`
 --> /tmp/demo.md
  |
3 | description: A really short example of an SIP.
  |
```
...
| id                                  | Description                                                                                   |
|-------------------------------------|-----------------------------------------------------------------------------------------------|
| `markdown-copyright`                | Final section contains the CC0-1.0 copyright waiver.                                          |
| `markdown-html-comments`            | There are no HTML comments in review-ready SIPs                                               |
| `markdown-json-cite`                | All `csl-json` code blocks adhere to the correct schema.                                      |
| `markdown-link-first`               | First mention of an SIP must be a link.                                                       |
| `markdown-link-status`              | SIPs linked in the body have statuses further along than the current proposal.                |
...
| `markdown-re-src-dash`              | Other SRCs are referenced using SRC-X, not SRCX or SRC X.                                     |
...
| `preamble-sip`                      | The `sip` header is a non-negative integer.                                                   |
...
| `preamble-file-name`                | The file name reflects the SIP number.                                                        |
...
| `preamble-re-description-sip-dash`  | SIPs referenced in the `description` header use a dash.                                       |
| `preamble-re-description-src-dash`  | SRCs referenced in the `description` header use a dash.                                       |
| `preamble-re-discussions-to`        | The `discussions-to` header points to Sila Magicians                                      |
...
| `preamble-re-title-sip-dash`        | SIPs referenced in the `title` header use a dash.                                             |
| `preamble-re-title-src-dash`        | SRCs referenced in the `title` header use a dash.                                             |
| `preamble-refs-description`         | SRCs referenced in the `description` header use the `SRC-` prefix.                            |
| `preamble-refs-title`               | SRCs referenced in the `title` header use the `SRC-` prefix.                                  |
...
| `preamble-requires-status`          | SIPs listed in `requires` have statuses further along than the current proposal.              |
...
`sipw-lint-js` packages `sipw` as an npm package, for use in JavaScript / TypeScript.

You can find the [package on npm](https://www.npmjs.com/package/sipw-lint-js).
