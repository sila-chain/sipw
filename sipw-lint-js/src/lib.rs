...
use sipw_lint::config::{DefaultOptions, Options, Override};
use sipw_lint::fetch::Fetch;
use sipw_lint::lints::DefaultLint;
use sipw_lint::modifiers::DefaultModifier;
use sipw_lint::reporters::{AdditionalHelp, Json};
...
pub async fn lint(sources: Vec<JsValue>, options: Option<Object>) -> Result<JsValue, JsError> {
    ...
    let reporter = AdditionalHelp::new(reporter, |t: &str| {
        Ok(format!("see https://sila.github.io/sipw/{}/", t))
    });
