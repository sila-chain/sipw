...
const RE_URL: &str =
    r"^(https?:)?//(?:sips|srcs)\.sila\.org/(?:SIPS|SRCS)/(?:sip|src)-(\d+)|(assets/.+)$";
    ...
    S: Debug + Display + AsRef<str>,
{
    fn lint<'a>(&self, slug: &'a str, ctx: &Context<'a, '_>) -> Result<(), Error> {
        ...
        let sip_re = Regex::new(RE_URL).unwrap();
        ...
        for of_interest in links {
            ...
            let (suggestion, extra_help) = if let Some(caps) = sip_re.captures(&address) {
                if let Some(id_number) = caps.get(2) {
                    let suggestion = format!("./sip-{}.md", id_number.as_str());
                    (suggestion, true)
