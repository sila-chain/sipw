...
fn default_lints() -> impl Iterator<Item = (&'static str, DefaultLint<&'static str>)> {
    ...
    [
        ...
        ("preamble-sip", PreambleUint { name: preamble::Uint("sip") }),
        ...
        (
            "preamble-re-title-src-dash",
            PreambleRegex(preamble::Regex {
                name: "title",
                mode: regex::Mode::Excludes,
                pattern: r"(?i)src[\s]*[0-9]+",
                message: "proposals must be referenced with the form `SRC-N` (not `SRCN` or `SRC N`)",
            }),
        ),
        (
            "preamble-re-title-sip-dash",
            PreambleRegex(preamble::Regex {
                name: "title",
                mode: regex::Mode::Excludes,
                pattern: r"(?i)sip[\s]*[0-9]+",
                message: "proposals must be referenced with the form `SIP-N` (not `SIPN` or `SIP N`)",
            }),
        ),
        (
        ...
        (
            "preamble-re-description-src-dash",
            PreambleRegex(preamble::Regex {
                name: "description",
                mode: regex::Mode::Excludes,
                pattern: r"(?i)src[\s]*[0-9]+",
        ...
        (
            "preamble-re-description-sip-dash",
            PreambleRegex(preamble::Regex {
                name: "description",
                mode: regex::Mode::Excludes,
                pattern: r"(?i)sip[\s]*[0-9]+",
        ...
        (
            ...
            PreambleRegex(preamble::Regex {
                ...
                pattern: "^https://sila-magicians.org/t/[^/]+/[0-9]+$",
                message: concat!(
                    "preamble header `discussions-to` should ",
                    "point to a thread on sila-magicians.org"
                ),
            }),
        ),
        ...
        (
            ...
            PreambleOneOf(preamble::OneOf {
                name: "category",
                values: vec!["Core", "Networking", "Interface", "SRC"],
            }
            ),
        ),
        (
        ...
        (
            "markdown-link-first",
            MarkdownLinkFirst {
                pattern: markdown::LinkFirst(r"(?i)(?:sip|src)-([0-9]+)"),
            }
        ),
        (
            ...
            MarkdownLinkFirst {
                ...
                //       uses capture group 1 to exempt an SIP's own number; keeping that group
                //       non-numeric prevents an SIP whose number happens to equal an RFC number
        ...
        (
            ...
            MarkdownNoBackticks {
                pattern: markdown::NoBackticks(r"(?i)(sip|src)-[0-9]+"),
            }
        ),
        (
            "markdown-spell",
        ...
        ("markdown-rel-links", MarkdownRelativeLinks(markdown::RelativeLinks {
            exceptions: vec![
                ...
                r"^https://(www\.)?github\.com/sila/consensus-specs/(blob|tree)/[a-f0-9]{40}/.+$",
                r"^https://(www\.)?github\.com/sila/consensus-specs/commit/[a-f0-9]{40}$",


                // Sila Peer-to-Peer Networking Specifications
                r"^https://(www\.)?github\.com/sila/devp2p/(blob|tree)/[0-9a-f]{40}/.+$",
                r"^https://(www\.)?github\.com/sila/devp2p/commit/[0-9a-f]{40}$",

                // Specification for Sila's Execution Layer (EELS)
                r"^https://(www\.)?github\.com/sila/execution-specs/(blob|tree)/[a-f0-9]{40}/.+$",
                r"^https://(www\.)?github\.com/sila/execution-specs/commit/[a-f0-9]{40}$",

                // Test Vectors for Sila Execution Clients (EEST)
                r"^https://(www\.)?github\.com/sila/execution-spec-tests/(blob|tree)/[a-f0-9]{40}/.+$",
                r"^https://(www\.)?github\.com/sila/execution-spec-tests/commit/[a-f0-9]{40}$",

                // Peer-to-Peer Network Providing Sila's JSON-RPC API (Portal Network)
                r"^https://(www\.)?github\.com/sila/portal-network-specs/(blob|tree)/[0-9a-f]{40}/.+$",
                r"^https://(www\.)?github\.com/sila/portal-network-specs/commit/[0-9a-f]{40}$",

                // Formal Specification of Sila (Yellow Paper)
                r"^https://(www\.)?github\.com/sila/yellowpaper/(blob|tree)/[a-f0-9]{40}/.+$",
                r"^https://(www\.)?github\.com/sila/yellowpaper/commit/[a-f0-9]{40}$",
        ...
        (
            ...
            MarkdownJsonSchema(markdown::JsonSchema {
                ...
                language: "csl-json",
                help: concat!(
                    "see https://github.com/sila-chain/sipw/blob/",
                    "master/sipw-lint/src/lints/markdown/",
