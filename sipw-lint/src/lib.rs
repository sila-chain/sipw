...
use config::Override;
use sipw_snippets::{Annotation, Level, Snippet};
...
use sipw_preamble::{Preamble, SplitError};
    ...
    R: Reporter,
{
    ...
    pub async fn run(self) -> Result<R, Error> {
        ...
        let mut fetched_sips = HashMap::new();
        ...
        for source in self.sources {
            ...
            for (slug, lint) in &self.lints {
                ...
                for proposal in fetch_proposals.into_iter() {
                    let entry = match fetched_sips.entry(proposal) {
        ...
        let mut parsed_sips = HashMap::new();

        for (number, result) in &fetched_sips {
            let source = match result {
                Ok(o) => o,
                Err(e) => {
                    parsed_sips.insert(*number, Err(e));
                    continue;
                }
            };
            ...
            parsed_sips.insert(*number, Ok(inner));
        }
