...
use sipw_snippets::{Level, Message, Snippet};
    ...
    'b: 'a,
{
    ...
    pub(crate) sips: &'b HashMap<u32, Result<InnerContext<'b>, &'b crate::Error>>,
    #[educe(Debug(ignore))]
    ...
    'b: 'a,
{
    ...
    pub fn proposal(&self, proposal: u32) -> Result<Context<'b, 'b>, &crate::Error> {
        let inner = match self.sips.get(&proposal) {
            Some(Ok(i)) => i,
            Some(Err(e)) => return Err(e),
            None => panic!("no sip found for key `{}`", proposal),
        };
...
mod tests {
    ...
    fn get_context_ast_source(source: &str, pred: impl FnMut(&Ast) -> bool) -> String {
        ...
        let context = Context {
            ...
            sips: &Default::default(),
            reporter: &crate::reporters::Null,
            inner: crate::process(&crate::reporters::Null, &arena, Some("sip-1234.md"), source)
                .unwrap()
                .unwrap(),
        };
